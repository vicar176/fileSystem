...
public class MainWindow {
	...
	private String tempConsoleContent;
	...
	public void executeCommand(String command, String instruccion) {
		...
		case "CREATE":
			...
			try {
				currentDir = commandsExecute.createVirtualDrive(
		...
		case "FILE":
			...
			try {
				...
				commandsExecute.createVirtualFile(content, name, ext,
						currentDir);
				setNodesToTree((DefaultMutableTreeNode) currentDir.getNode()
						.getRoot());
		...
		case "MKDIR":
			try {
				currentDir = commandsExecute.createVirtualDirectory(
						instruccion, currentDir);
				setNodesToTree((DefaultMutableTreeNode) currentDir.getNode()
						.getRoot());
		...
		case "ListarDIR":
			try {
				...
				Map<String, List<String>> dirContentMap = commandsExecute
						.listDirContent(currentDir);
		...
		case "ModFILE":
			try {
				String content = commandsExecute.verContenido(instruccion, currentDir);
				tempConsoleContent = consola.getText();
				consola.setText(content);
		...
		case "VerPropiedades":
			try {
				String output = commandsExecute.verPropiedades(instruccion,
						currentDir);
		...
		case "ContFile":
			try {
				String output = commandsExecute.verContenido(instruccion,
						currentDir);
				setConsoleText(output);
				lineaComandos.setText("");
			} catch (Exception e) {
		...
		case "CoPY":
			try {
				// para copiar hay que estar en el directorio donde esta el
	...
	private void guardarContenido(){
		btnGuardar.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				String fileName = lineaComandos.getText().split(";")[1];
				String content = consola.getText();
				try {
					commandsExecute.actualizaContenido(fileName, currentDir, content);
				} catch (Exception e1) {
					e1.printStackTrace();
					String output = "No se pudo guardar el contenido en el archivo\n";
					tempConsoleContent = output + tempConsoleContent;
				}
				consola.setText(tempConsoleContent);
	...
	private void initialize() {
		...
		frame.getContentPane().setLayout(null);
				...
				btnGuardar.setBounds(607, 54, 71, 23);
				btnGuardar.setVisible(false);
				frame.getContentPane().add(btnGuardar);

		path = new JLabel("");
		path.setToolTipText("Posici\u00F3n Actual");
		path.setBounds(133, 58, 545, 14);
